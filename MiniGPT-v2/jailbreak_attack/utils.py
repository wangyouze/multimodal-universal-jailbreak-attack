...
def evaluate_batch(model, batch_goals, trigger, snli=False):
    """
    ...
    trigger_sequence_tensor = trigger_sequence_tensor.repeat(batch_goals_token_ids.shape[0], 1).cuda()
    
    original_tokens = batch_goals_token_ids.clone()
    batch_goals_token_ids = torch.cat((trigger_sequence_tensor, original_tokens), 1)
...
def get_average_grad(model, batch_goals, trigger, target_label=None, snli=False):
    """
    ...
    loss = evaluate_batch(model, batch_goals_token_ids, trigger_token_ids, snli)['loss']
    loss.backward()
